...
# ─── Ledgrrr Viz Dashboard Server ─────────────────────────────────────────────
#
# Start the HTTP + CDP-compatible viz dashboard for process visualization.
# Requires: vendor/ledgrrr checkout (b00t ledgrrr install)
# Dashboard HTML auto-generated by holon-viz-demo binary.
#
# Usage:
#   just ledgrrr-viz              # start on :8080 (default)
#   just ledgrrr-viz PORT=9090    # custom port
#   just ledgrrr-viz-stop         # stop the server
#   just ledgrrr-viz-status       # check if running
ledgrrr-viz PORT="8080":
    b00t ledgrrr viz start --port={{PORT}}

ledgrrr-viz-stop:
    b00t ledgrrr viz stop

ledgrrr-viz-status:
    b00t ledgrrr viz status

# Regenerate the dashboard HTML from the holon-viz crate
ledgrrr-viz-build:
    cd vendor/ledgrrr && cargo run -p holon-viz --bin holon-viz-demo

# Run the comprehensive 42-test MECE harness for holon-viz
ledgrrr-test:
    cd vendor/ledgrrr/crates/holon-viz && cargo test --test holon_viz_comprehensive

# Run ALL holon-viz tests (unit + integration)
ledgrrr-test-all:
    cd vendor/ledgrrr/crates/holon-viz && cargo test

# ─── Ledgrrr MCP Server Management ────────────────────────────────────────────
ledgrrr-status:
    b00t ledgrrr status

ledgrrr-install:
    b00t ledgrrr install
