...
2. Spawn a private Linux namespace (via the upcoming c-containers service) that bind-mounts only the folders required for that role: `skills/matlab`, `learn/matlab`, `lfmf/matlab`, etc. The namespace can use `scripts/rustfs-skill.sh <skill> start` to mirror the chosen skill tree into a RustFS container, optionally relying on `s3fs-fuse` when `SKILL_BUCKET` is set.
...
5. Use `b00t stack ansible` or `b00t stack list` (and `scripts/rustfs-skill.sh <skill> start`) to ensure the necessary datums are installed before starting the containerized role.
