...
```bash
cd /home/brianh/.dotfiles/.claude/worktrees/animate-x-transmogrify
...
**Correction found during implementation (verified against the real `~/promptexecution/animate-x` source, not just the README):** `inference.py` takes no `--source_image`/`--output_dir` CLI flags — every per-run parameter comes from the YAML config's `test_list_path`/`log_dir`/`test_model` keys, so a per-job config must be generated. Also, `dwpose/wholebody.py` hardcodes onnx checkpoint paths as `checkpoints/yolox_l.onnx` and `checkpoints/dw-ll_ucoco_384.onnx` relative to cwd (not configurable), and the config's default `test_model: checkpoints/animate-x_ckpt.pth` doesn't match the published checkpoint filename (`animate-x.pth` per `checkpoint_manifest.py`). The implementation below accounts for both.
...
def run_animate_x_job(
    ...
    *,
    ...
    # process_data.py names its outputs after the motion video's basename
    # (see its main(): save_pkl_path uses os.path.basename(file_path)[:-4]).
...
- [ ] **Step 2: Add `pyyaml` and verify the module imports**

Run: `cd /home/brianh/.dotfiles/animate-x-service && . .venv/bin/activate && uv pip install pyyaml && python -c "import pipeline"`
Expected: no output from the import (success); add `pyyaml>=6.0` to `requirements-service.txt` in Task 5.
