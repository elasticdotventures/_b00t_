...
| **D-SP5-2** | **Refactor, do not duplicate.** | `compile_datum_triples` (currently `b00t-cli/src/datum_triples.rs`) **moves into `b00t-c0re-lib`**; `b00t-cli` keeps a `pub use` shim for its `reasoning::*` callers. The generic graph→KerML text emitter is **contributed to `ufo-types`** (`sysml_model`), consumed by SP5 *and* by `dispatch_sysml.rs` (which drops its private `dispatch_chain_to_sysml_v2` hand-roll) *and* available to kr0ki/systhread later. No second copy of any walk or emitter. |
...
- **`b00t-cli/src/datum_triples.rs::compile_datum_triples(b00t_path) ->
  ...
  `rdfs:label`. **Moves to `b00t-c0re-lib` in SP5-06.**
...
|---|---|---|---|---|---|
| **SP5-06** | **Refactor** `compile_datum_triples` into `b00t-c0re-lib` | `b00t-c0re-lib/src/datum_triples.rs` (moved), `b00t-cli/src/datum_triples.rs` → `pub use b00t_c0re_lib::datum_triples::*;` shim | Function + `emit_ref_list` helper move verbatim; `b00t-cli`'s `reasoning::{graph_rules,adjacency}` callers unchanged via the shim. Needs `datum_utils::get_all_datums` reachable from `b00t-c0re-lib`; if it is b00t-cli-only, either lift the minimal loader or change the signature to `compile_datum_triples(datums: &[(String, BootDatum)])`. **No duplicated walk.** | — | M |
