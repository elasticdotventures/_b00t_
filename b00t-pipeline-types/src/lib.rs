//! Lightweight standalone pipeline types extracted from b00t-cli.
//!
//! Contains StageSpec, CapsuleProfile, StagePort, ResourceRequirements,
//! HostResources, ResourceFit, PipelineDag, PipelineError, and supporting types.
//! Includes SecretSource/SecretRef from pipeline_secrets.rs and FlowStrategy/
//! StageFlowConfig from pipeline_flowctl.rs — enough surface to construct and
//! hand off a StageSpec without pulling in async-nats, K8s, SSH, or other heavy
//! b00t-cli runtime deps.
//!
//! Extraction tracked in gh #1251.
...
// ── FlowStrategy & StageFlowConfig (from pipeline_flowctl.rs) ──
...
// ── Pipeline types (from pipeline_types.rs) ──
...
// ── GH #780: Satisfies<ResourceRequirements> bridge ──
...
pub struct StageSpec {
    ...
    pub flow_control: Option<StageFlowConfig>,
}
...
impl StageSpec {
    ...
    pub fn with_flow_control(mut self, strategy: FlowStrategy) -> Self {
        self.flow_control = Some(StageFlowConfig::new(&self.name, strategy));
        self
    }
}
...
mod tests {
    ...
    fn satisfies_resource_requirements_produces_passing_evidence() {
        ...
        let host = HostResources {
            ram_gb: 8.0, vram_gb: 0.0, gpu_count: 0, cpu_cores: 4,
        };
    ...
    fn satisfies_resource_requirements_produces_violated_evidence() {
        ...
        let req = ResourceRequirements {
            min_ram_gb: 64.0, min_vram_gb: 0.0, requires_gpu: false,
            cpu_cores: None, scratch_disk_gb: None,
        };
        let host = HostResources {
            ram_gb: 8.0, vram_gb: 0.0, gpu_count: 0, cpu_cores: 4,
    ...
    fn satisfies_resource_requirements_agrees_with_fits_on() {
        ...
        let req = ResourceRequirements {
            min_ram_gb: 1.0, min_vram_gb: 8.0, requires_gpu: true,
            cpu_cores: None, scratch_disk_gb: None,
        };
        let host = HostResources {
            ram_gb: 16.0, vram_gb: 16.0, gpu_count: 1, cpu_cores: 8,
        };
