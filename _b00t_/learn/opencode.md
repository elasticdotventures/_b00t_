...
---
non-interactive 'opencode run' dispatch auto-rejects external_directory permissions and the session dies mid-task: non-interactive 'opencode run' dispatch auto-rejects external_directory permissions and the session dies mid-task (deepseek-v4-flash died reading ~/.cargo/registry crate sources); instruct delegated agents to stay inside the repo and rely on compiler errors/docs instead of reading dependency sources <!-- salvaged:no_colon -->
