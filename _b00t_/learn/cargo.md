...
---
worktree-shared-target-dir: Point CARGO_TARGET_DIR (or [build] target-dir in .cargo/config.toml) at one location shared across every git worktree of the same repo. Dependency compilation is the expensive, reusable part (often several GB); without sharing, each throwaway worktree rebuilds it from scratch.
