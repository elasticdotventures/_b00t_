...
fn flange() -> Result<Solid, Box<dyn std::error::Error>> {
    ...
    for i in 0..6 {
        ...
        let hole = Solid::cylinder(3.0, DVec3::Z * 35.0)
            .translate(DVec3::new(10.0 * angle.cos(), 10.0 * angle.sin(), -2.5));
    ...
    let base = Solid::cube(DVec3::ZERO, DVec3::new(40.0, 40.0, 5.0))
        .translate(-DVec3::new(20.0, 20.0, 0.0)).color("#e8a87c");
    Ok((&part + &base).build()?)
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let name = "flange";
    Solid::write_step([&solid], &mut BufWriter::new(std::fs::File::create(format!("{name}.step"))?))?;
    mesh.write_stl(&mut BufWriter::new(std::fs::File::create(format!("{name}.stl"))?))?;
    mesh.write_gltf_binary(&mut BufWriter::new(std::fs::File::create(format!("{name}.glb"))?))?;

    println!("{name} — {:.2?} | {} tris | .step .stl .glb", start.elapsed(), mesh.vertices.len());
    Ok(())
}
