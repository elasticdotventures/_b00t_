...
fn test_generate_kube_yaml_contains_labels() {
    ...
    let yaml = generate_kube_yaml("a/b", "linux,x64,self-hosted", "/tmp/w", false, "/var/run/docker.sock");
...
fn test_generate_kube_yaml_ephemeral_true() {
    ...
    let yaml = generate_kube_yaml("a/b", "l", "/tmp/w", true, "/var/run/docker.sock");
...
fn test_generate_kube_yaml_ephemeral_false() {
    ...
    let yaml = generate_kube_yaml("a/b", "l", "/tmp/w", false, "/var/run/docker.sock");
...
fn test_generate_kube_yaml_socket_none() {
    ...
    let yaml = generate_kube_yaml("a/b", "l", "/tmp/w", false, "");
    assert!(!yaml.contains("docker-sock"));
}
...
fn test_generate_kube_yaml_has_workdir_volume() {
    ...
    let yaml = generate_kube_yaml("x/y", "l", "/tmp/mywork", false, "");
...
fn test_generate_kube_yaml_contains_restart_policy() {
    ...
    let yaml = generate_kube_yaml("x/y", "l", "/tmp/w", false, "");
    assert!(yaml.contains("restartPolicy: OnFailure"));
}

#[test]
fn test_generate_kube_yaml_contains_secret_keyref() {
    // ✅ Positive: token via secretKeyRef, not hardcoded
    let yaml = generate_kube_yaml("x/y", "l", "/tmp/w", false, "");
    assert!(yaml.contains("secretKeyRef:"));
    assert!(yaml.contains("name: gh-runner-token-x-y"));
}
...
fn test_generate_kube_yaml_runner_name_includes_hostname() {
    ...
    let yaml = generate_kube_yaml("myorg/myrepo", "l", "/tmp/w", false, "");
