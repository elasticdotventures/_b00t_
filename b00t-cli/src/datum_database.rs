...
impl DatabaseDatum {
    pub fn from_config(name: &str, path: &str) -> Result<Self> {
        ...
        Ok(DatabaseDatum { datum: config.b00t })
    }
            ...
            match self.get_db_type().as_deref() {
                ...
                Some("redis") => {
                    ...
                    if let Ok(url) = Url::parse(dsn) {
                        ...
                        let result =
                            cmd!("redis-cli", "-h", host, "-p", &port.to_string(), "ping").read();
                        result
                            .map(|output| output.trim() == "PONG")
                            .unwrap_or(false)
                    } else {
                        false
                    }
...
impl DatumCreator for DatabaseDatum {
    fn create_interactive(name: &str, _path: &str) -> Result<BootDatum> {
        ...
        let dsn = match db_type.as_str() {
            ...
            "sqlite" => {
                ...
                let path = path_result
                    .unwrap_or_else(|_| format!("./{}.db", name))
                    .trim()
                    .to_string();
        ...
        let hint = hint_result
            .unwrap_or_else(|_| format!("{} database: {}", db_type, name))
            .trim()
            .to_string();
