...
impl Orchestrator {
    ...
    fn extract_datum_name(filename: &str) -> Option<String> {
        ...
        // Remove type suffix (e.g., .docker, .mcp, .cli)
        let types = ["docker", "mcp", "cli", "bash", "vscode", "k8s", "apt", "ai", "stack"];
        for typ in types {
    ...
    pub async fn ensure_dependencies(&self, datum_key: &str) -> Result<Vec<String>> {
        ...
        // Get dependency keys from depends_on field
        let dep_keys = datum.depends_on.clone().unwrap_or_default();

        // Start each dependency if needed
