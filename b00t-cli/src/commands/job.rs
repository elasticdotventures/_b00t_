            ...
            match result {
                ...
                Err(e) => {
                    ...
                    if attempt <= max_retries {
                        println!("   ⚠️  Attempt {}/{} failed: {}", attempt, max_retries + 1, e);
                        println!("   🔄 Retrying...");
                        last_error = Some(e);
                        continue;
                    }

                    println!("   ❌ Failed after {} attempts: {}", attempt, e);
                    ...
                    return Err(e.context(format!("Step '{}' failed after {} attempts", step_name, attempt)));
                }
            }
        }
    }
