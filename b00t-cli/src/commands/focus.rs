//! `b00t focus` — query persisted FOCUS records from ledgerr-mcp JSONL files.
//!
//! FOCUS records are written to `focus_records.jsonl` by the ledgerr-mcp server.
    ...
    match args {
        ...
        FocusCommands::Export {
            ...
            let export_data: String = match format.as_str() {
                ...
                other => {
                    return Err(anyhow!(
                        "unsupported format '{other}'. Use: json, t00n, csv"
                    ));
                }
            };
        ...
        FocusCommands::Merge {
            ...
            let mut secondary_seq = FocusJsonlSequence::open(&secondary_str).map_err(|e| {
                anyhow!("failed to open secondary '{}': {}", secondary.display(), e)
            })?;
