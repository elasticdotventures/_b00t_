...
fn check_bashrc(path: &str) {
    ...
    let mut bashrc_path = crate::get_expanded_path(path).unwrap_or_else(|_| std::path::PathBuf::from(path));
    bashrc_path.push("_b00t_.bashrc");

    if !bashrc_path.exists() {
        eprintln!("⚠️ _b00t_.bashrc missing at {} — run install.sh or copy from .dotfiles/_b00t_/_b00t_.bashrc", bashrc_path.display());
        return;
    }
