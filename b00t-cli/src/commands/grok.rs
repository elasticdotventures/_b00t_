...
async fn handle_assimilate(
    ...
    let git_repo_root = find_git_root()?;
...
#    Retrieve: git -C {git_repo_path} cat-file blob {git_hash} | gunzip
...
[validation]
validate  = "git -C {git_repo_path} cat-file -e {git_hash}"
# 🤓 Decompress: git -C {git_repo_path} cat-file blob {git_hash} | gunzip
...
# tags: {tags_str}
# tier: sm0l
# cmds: git -C {git_repo_path} cat-file blob {git_hash} | gunzip
# complexity: 1
"#,
