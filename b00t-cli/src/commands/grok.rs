...
async fn handle_assimilate(
    ...
    let (git_hash, git_repo_root) = store_as_git_blob(&content)?;
    ...
    let git_repo_path = git_repo_root.display().to_string();
...
#    Retrieve: git -C {git_repo_path} cat-file blob {git_hash}
#    Validate: git -C {git_repo_path} cat-file -e {git_hash}
...
# tags: {tags_str}
# tier: sm0l
# cmds: git -C {git_repo_path} cat-file blob {git_hash}
# complexity: 1
"#,
...
/// Store content as a git blob object — returns (SHA1 hash, repo_root used)
/// 🤓 Uses `git -C <repo_root> hash-object -w --stdin` to ensure
///    the blob lands in the correct repo's object store
fn store_as_git_blob(content: &str) -> Result<(String, std::path::PathBuf)> {
    ...
    // Pin the blob with a loose ref so `git gc` cannot prune it
    keep_blob_reachable(&repo_root, &hash);

    Ok((hash, repo_root))
}

/// Pin blob with a `refs/b00t-datums/<hash>` loose ref so `git gc` cannot prune it.
///
/// Non-fatal: if the ref cannot be created (permission errors, detached HEAD, read-only repo),
/// a warning is printed but the blob hash is still returned. The blob remains accessible until
/// the next `git gc --prune=now` run. Use `git update-ref refs/b00t-datums/<hash> <hash>`
/// manually to re-pin if needed.
fn keep_blob_reachable(repo_root: &std::path::Path, hash: &str) {
    let ref_name = format!("refs/b00t-datums/{}", hash);
    let status = std::process::Command::new("git")
        .args(["-C", repo_root.to_str().unwrap_or("."), "update-ref", &ref_name, hash])
        .status();
    match status {
        Ok(s) if s.success() => {}
        Ok(_) | Err(_) => {
            eprintln!(
                "  ⚠️  Could not pin blob ref refs/b00t-datums/{} — git gc may prune datum.\n       \
                 To prevent loss, run: git -C {} update-ref {} {}",
                hash, repo_root.display(), ref_name, hash
            );
        }
    }
}
