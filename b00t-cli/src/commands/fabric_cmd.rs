...
use b00t_c0re_lib::irontology_bridge::{
    EdgeKind, EdgeRecord, FactRecord, IrontologyBridgeClient, compiled_knowledge_backend,
...
pub enum FabricCommands {
    ...
    Edge {
        ...
        #[clap(long, default_value = "RelatedTo")]
        kind: String,
    ...
    ScanTraits {
        ...
        path: std::path::PathBuf,
        #[clap(long, default_value = "traits")]
        namespace: String,
    },
    ...
    Adjacent {
        ...
        #[clap(long)]
        goal: String,
        #[clap(long, default_value = "autolearn")]
        namespace: String,
...
pub fn handle_fabric_command(args: &FabricCommands) -> Result<()> {
    ...
    tokio::task::block_in_place(|| tokio::runtime::Handle::current().block_on(fabric_async(args)))
}

async fn fabric_async(args: &FabricCommands) -> Result<()> {
    match args {
        FabricCommands::Query {
            subject,
            predicate,
            object,
            namespace,
            ...
            let mut facts = client
                .query_triples(subject.clone(), predicate.clone())
                .await?;
                ...
                "table" => {
                    ...
                    for f in &facts {
                        println!(
                            "{:<40} {:<30} {}",
                            trunc(&f.subject, 38),
                            trunc(&f.predicate, 28),
                            trunc(&f.object.to_string(), 40)
                        );
                    }
                    println!("\n{} records", facts.len());
                }
        ...
        FabricCommands::Upsert {
            ...
            let obj_val: serde_json::Value =
                serde_json::from_str(object).unwrap_or(serde_json::Value::String(object.clone()));
            client
        ...
        FabricCommands::Status { namespace } => {
            ...
            if let Ok(client) = IrontologyBridgeClient::new(namespace) {
                match client.query_triples(None, None).await {
                    Ok(facts) => {
                        println!("   triples : {} (scan capped at store limit)", facts.len())
                    }
        ...
        FabricCommands::Reason {
            ...
            if show_all || relation == "reachable" {
                print_pairs("reachable", &result.horn.reachable);
            }
            if show_all || relation == "depends_on" {
                print_pairs("depends_on", &result.horn.depends_on);
            }
            if show_all || relation == "informed_by" {
                print_pairs("informed_by", &result.horn.informed_by);
            }
            ...
            if show_all || relation == "shortest_path" {
                ...
                if format.as_str() == "json" {
                    let v: Vec<_> = paths
                        .iter()
                        .map(|(f, t, d)| json!({"from":f,"to":t,"hops":d}))
                        .collect();
                    println!("{}", serde_json::to_string_pretty(&v).unwrap_or_default());
                } else {
        ...
        FabricCommands::ScanTraits { path, namespace } => {
            ...
            let n = triples.len();
            let client = IrontologyBridgeClient::new(namespace)?;
            let facts: Vec<FactRecord> = triples
                .into_iter()
                .map(|(s, p, o)| FactRecord {
                    subject: s,
                    predicate: p,
            ...
            client.upsert_facts(facts).await?;
            println!(
                "✅ scanned {n} trait triples from {} → ns={namespace}",
                path.display()
            );
            Ok(())
        }

        FabricCommands::Adjacent {
            goal,
            namespace,
            top,
            format,
            ...
            let goal_words: Vec<&str> = goal.split_whitespace().filter(|w| w.len() > 2).collect();
            ...
            match format.as_str() {
                ...
                "context" => {
                    ...
                    println!(
                        "  Skills reachable via b00t:dependsOn from high-scoring skills inherit relevance."
                    );
                }
                _ => {
                    for (skill, score) in &adjacent {
