...
fn get_config_with_type_preference(
    ...
    // 🤓 base_suffix() is generated by define_datum_types! macro — single source of truth.
    //    get_config_typed tries .tomllm first, then .toml automatically.
    let expanded_path = get_expanded_path(path)?;
    if let Some(base) = expected_type.base_suffix() {
