...
impl RuntimeAdapter for ClaudeAdapter {
    ...
    fn register_hooks(
        ...
        if !ctx.content_packs.contains(&ContentPackId::Hooks) {
            return Ok(());
        }
        ...
        if !fragment_path.exists() {
            eprintln!(
                "⚠️  No settings_fragment.json for Claude runtime — skipping hook registration"
            );
            return Ok(());
        }
...
mod tests {
    ...
    fn test_claude_target_dir_global() {
        ...
        let target = adapter
            .target_dir(&InstallScope::Global)
    ...
    fn test_claude_target_dir_local() {
        ...
        let target = adapter
            .target_dir(&InstallScope::Local(project.clone()))
    ...
    fn test_claude_install_creates_manifest() {
        ...
        let adapter = ClaudeAdapter;
        let config = Arc::new(ClaudeConfig {
    ...
    fn test_register_hooks_produces_valid_json() {
        ...
        let mut manifest = B00tInstallManifest::new(
            RuntimeId::Claude,
            InstallScope::Local(tmp.path().to_path_buf()),
        );
    ...
    fn test_register_hooks_merges_into_existing_settings() {
        ...
        let content = std::fs::read_to_string(tmp.path().join("settings.json")).unwrap();
        let parsed: serde_json::Value =
            serde_json::from_str(&content).expect("settings.json must be valid JSON after merge");
    ...
    fn test_uninstall_removes_note_key() {
        ...
        let parsed: serde_json::Value = serde_json::from_str(&content).unwrap();
        assert!(
            parsed.get("_note").is_none(),
            "_note must be removed on uninstall"
        );
