...
pub trait RuntimeAdapter: Send + Sync {
    ...
    fn register_hooks(
        &self,
        ctx: &InstallContext,
        manifest: &mut super::manifest::B00tInstallManifest,
    ) -> Result<()>;
}
...
mod tests {
    ...
    impl RuntimeConfig for TestAdapter {
        fn settings_path(&self) -> PathBuf {
            PathBuf::from("/tmp/test/settings.json")
        }
        fn hooks_dir(&self) -> PathBuf {
            PathBuf::from("/tmp/test/hooks")
        }
        fn agents_dir(&self) -> PathBuf {
            PathBuf::from("/tmp/test/agents")
        }
        fn skills_dir(&self) -> PathBuf {
    ...
    impl RuntimeAdapter for TestAdapter {
        ...
        fn target_dir(&self, _scope: &InstallScope) -> Result<PathBuf> {
        ...
        fn install(
            &self,
            _ctx: &InstallContext,
        ) -> Result<super::super::manifest::B00tInstallManifest> {
            Ok(super::super::manifest::B00tInstallManifest::new(
                self.id(),
                super::InstallScope::Global,
            ))
        }
        fn uninstall(&self, _manifest: &super::super::manifest::B00tInstallManifest) -> Result<()> {
            Ok(())
        }
        fn register_hooks(
            &self,
            _ctx: &InstallContext,
            _manifest: &mut super::super::manifest::B00tInstallManifest,
        ) -> Result<()> {
            Ok(())
        }
    }
