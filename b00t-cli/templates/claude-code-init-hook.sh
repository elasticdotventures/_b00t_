...
echo "🥾 Initializing b00t for Claude Code..."
echo

# 1. Ensure b00t virtfs is mounted
if ! mountpoint -q ~/.claude/b00t 2>/dev/null; then
    echo "📁 Mounting b00t virtual filesystem..."

    if command -v b00t-cli &> /dev/null; then
        b00t-cli mount --daemon
        echo "   ✅ Mounted at ~/.claude/b00t"
    else
        echo "   ⚠️  b00t-cli not found, virtfs not mounted"
        echo "   Install: cd ~/.b00t/b00t-cli && cargo install --path ."
    fi
else
    echo "✅ b00t virtfs already mounted"
fi
echo
...
EOF
    ...
    echo "⚠️  b00t-cli not available, skipping MCP setup"
fi
echo
...
if [ -f ~/.b00t/b00t-cli/target/release/b00t-cli ]; then
    ...
    if mountpoint -q ~/.claude/b00t 2>/dev/null; then
        ...
        echo "   ✅ Generated $(ls $COMMANDS_DIR/*.md 2>/dev/null | wc -l) slash commands"
    else
        echo "   ⚠️  virtfs not mounted, skipping command generation"
    fi
else
    echo "   ⚠️  b00t-cli not built, skipping command generation"
fi
echo
...
if [ ! -f "$PROJECT_ROOT/.b00trc" ]; then
    echo "⚙️  Creating project .b00trc..."
...
echo
echo "✅ b00t initialization complete!"
echo
echo "Next steps:"
