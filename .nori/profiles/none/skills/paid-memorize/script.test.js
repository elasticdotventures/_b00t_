...
describe("paid-memorize script", () => {
    ...
    describe("tier checking", () => {
        ...
        it("should fail with error message when config has no auth credentials", async () => {
            ...
            await expect(main()).rejects.toThrow("process.exit(1)");
            expect(processExitSpy).toHaveBeenCalledWith(1);
            expect(consoleErrorSpy).toHaveBeenCalledWith("Error: This feature requires a paid Nori subscription.");
        });
    });
    describe("argument parsing", () => {
        it("should fail when --name is missing", async () => {
            ...
            await expect(main()).rejects.toThrow("process.exit(1)");
            expect(processExitSpy).toHaveBeenCalledWith(1);
            expect(consoleErrorSpy).toHaveBeenCalledWith("Error: --name parameter is required");
        });
        it("should fail when --content is missing", async () => {
            ...
            await expect(main()).rejects.toThrow("process.exit(1)");
            expect(processExitSpy).toHaveBeenCalledWith(1);
            expect(consoleErrorSpy).toHaveBeenCalledWith("Error: --content parameter is required");
        });
