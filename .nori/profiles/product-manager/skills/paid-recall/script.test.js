...
describe("paid-recall script", () => {
    ...
    describe("installDir resolution", () => {
        it("should find config in parent directory when running from subdirectory", async () => {
            ...
            expect(consoleErrorSpy).not.toHaveBeenCalledWith("Error: This feature requires a paid Nori subscription.");
            // Cleanup
            await fs.rm(parentDir, { recursive: true, force: true });
        });
        it("should fail with clear error when no installation found", async () => {
            // Create directory with NO config file
            const noInstallDir = path.join(os.tmpdir(), `recall-no-install-${Date.now()}`);
            await fs.mkdir(noInstallDir, { recursive: true });
            process.cwd = () => noInstallDir;
            process.argv = ["node", "script.js", "--query=test"];
            await expect(main()).rejects.toThrow("process.exit(1)");
            expect(processExitSpy).toHaveBeenCalledWith(1);
            expect(consoleErrorSpy).toHaveBeenCalledWith("Error: No Nori installation found.");
            // Cleanup
    ...
    describe("argument parsing", () => {
        it("should fail when --query is missing", async () => {
            ...
            await expect(main()).rejects.toThrow("process.exit(1)");
            expect(processExitSpy).toHaveBeenCalledWith(1);
            expect(consoleErrorSpy).toHaveBeenCalledWith("Error: Either --query or --id parameter is required");
        });
    ...
    describe("--id parameter", () => {
        it("should fail when both --id and --query are provided", async () => {
            ...
            await expect(main()).rejects.toThrow("process.exit(1)");
            expect(processExitSpy).toHaveBeenCalledWith(1);
            expect(consoleErrorSpy).toHaveBeenCalledWith("Error: --id and --query are mutually exclusive. Provide one or the other.");
        });
