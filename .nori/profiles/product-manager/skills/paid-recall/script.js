...
// build/src/cli/features/claude-code/profiles/config/product-manager/skills/paid-recall/script.js
...
var formatArtifact = (args) => {
  ...
  return `${index + 1}. ${artifact.name}
   ...
   Content: ${artifact.content.length > 500 ? artifact.content.substring(0, 500) + "..." : artifact.content}`;
};
var formatFullArtifact = (args) => {
  const { artifact } = args;
...
var main = async () => {
  ...
  if (args.id != null && args.query != null) {
    console.error("Error: --id and --query are mutually exclusive. Provide one or the other.");
    process.exit(1);
  }
  if (args.id == null && args.query == null) {
  ...
  if (args.id != null) {
    ...
    const formattedResults = result.results.map((artifact, index) => formatArtifact({ artifact, index })).join("\n\n");
    let sourcesInfo = "";
    if (result.sources) {
      const sources = [];
...
Search sources: ${sources.join(", ")}`;
    ...
    console.log(`Found ${result.results.length} artifacts matching "${query}":
