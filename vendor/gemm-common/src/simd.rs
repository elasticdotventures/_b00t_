...
pub mod aarch64 {
    ...
    pub unsafe fn vfmaq_laneq_f16<const LANE: i32>(
        ...
        // Broadcast element LANE of c across all lanes, then regular FMLA.
        // Uses only FEAT_FP16 instructions — no indexed FMLA (FEAT_FP16FML) required.
        let lane: float16x8_t;
        asm!(
            "dup {0:v}.8h, {1:v}.h[{2}]",
            out(vreg) lane,
            in(vreg) c,
