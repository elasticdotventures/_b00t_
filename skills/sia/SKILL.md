...
name: sia
type: skill
hint: SIA: Self Improving AI — 3-agent loop (Meta→Target→Feedback) for autonomous benchmark improvement
version: 1.0.0
tags: [self-improving, ai, agents, benchmark, meta-agent, feedback-loop, harness, weight-updates, mle-bench, lawbench, python, ml]
tier: ch0nky
complexity: 6
description: |-
  SIA Self Improving AI — Meta→Target→Feedback 3-agent loop for autonomous task improvement. 56.6% LawBench gain, #1 MLE-Bench, 14x GPU kernel speedup.
---
...
1. Install: `pip install 'sia-agent[claude]'`
2. Run a task: `sia run --task gpqa --max_gen 5 --run_id 1`
3. Visualize results: `sia web`
4. For custom tasks: `sia run --task_dir ./my-task --max_gen 5 --run_id 1`
5. For MLE-Bench: prepare dataset with `python -m sia.prepare_mlebench_dataset -c 'spaceship-titanic'`

<!-- b00t:map v1
summary: SIA Self Improving AI — Meta→Target→Feedback 3-agent loop for autonomous task improvement
tags: self-improving, ai, agents, python, ml, benchmark
tier: ch0nky
cmds: pip install 'sia-agent[claude]'; sia run --task gpqa --max_gen 3 --run_id 1
complexity: 6
-->
