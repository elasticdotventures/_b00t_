...
pub fn query(needle: &str, limit: usize) -> Vec<Recommendation> {
    let manifest: BlessedManifest = toml::from_str(BLESSED_TOML).unwrap_or(BlessedManifest { crates: vec![] });
    let needle_lower = needle.to_lowercase();
    let mut scored: Vec<(usize, &CrateEntry)> = manifest.crates.iter()
        .filter_map(|e| {
            let mut score = 0usize;
            let ul = e.use_case.to_lowercase();
            let cl = e.category.to_lowercase();
            let rl = e.recommended.join(" ").to_lowercase();
            if ul.contains(&needle_lower) { score += 10; }
            for word in needle_lower.split_whitespace() {
                if ul.contains(word) { score += 5; }
                if cl.contains(word) { score += 2; }
                if rl.contains(word) { score += 1; }
            }
    ...
    scored.iter().map(|(_, e)| Recommendation {
        category: e.category.clone(),
        use_case: e.use_case.clone(),
        crates: e.recommended.iter().map(|name| CrateInfo {
            name: name.clone(),
...
mod tests {
    ...
    fn test_query_http() {
        ...
        let has_http = results.iter().any(|r| r.use_case.to_lowercase().contains("http"));
    ...
    fn test_query_random() {
        ...
        assert!(!results.is_empty());
        let has_rand = results.iter().any(|r| r.crates.iter().any(|c| c.name == "rand"));
        assert!(has_rand);
    }
}
